      ...
      return {
        ...
        relationship_status_to_founder:
          typeof item.relationship_status_to_founder === "string"
            ...
            : "",
        reports_to_role_id: typeof item.reports_to_role_id === "string" ? item.reports_to_role_id : "",
        role_id: roleId,
        role_name: roleName,
      };
    })
...
function buildExecutionBlockPayload(
  ...
  next.top_bottlenecks = payload.bottlenecks.map((item) => item.trim()).filter((item) => item.length > 0);
  next.focus_areas = payload.focusAreas.map((item) => item.trim()).filter((item) => item.length > 0);
